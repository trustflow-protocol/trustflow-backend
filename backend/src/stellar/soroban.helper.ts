...
export async function withSorobanFailover<T>(
  ...
  const endpoints = primaryEndpoint 
    ? [primaryEndpoint, ...(process.env.SOROBAN_RPC_ENDPOINTS || '').split(',').filter(Boolean).map(url => url.trim())]
    : (process.env.SOROBAN_RPC_ENDPOINTS || 'https://soroban-testnet.stellar.org').split(',').map(url => url.trim());
